...
export function HtmlEditor({ value, onChange, label, height = 400 }: HtmlEditorProps) {
  ...
  const wrapSelection = useCallback(
    (before: string, after: string) => {
      ...
      const start = el.selectionStart;
      const end = el.selectionEnd;
      const selected = value.substring(start, end) || 'tekst';
    ...
    <div className="flex flex-col gap-2">
      ...
      {activeTab === 'edit' ? (
        ...
        >
          ...
          <iframe
            srcDoc={value || '<p style="padding:20px;color:#94a3b8">Geen inhoud…</p>'}
